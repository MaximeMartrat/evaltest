...
def verifyLog():
    if request.method == 'GET':
        ...
        with mysql.connector.connect(**connection_params) as db:
            with db.cursor() as c:
                c.execute(queries_general.get_hashed_password, {'pseudo': pseudo})
                result = c.fetchone()
                ...
                if result is not None:
                    ...
                    if bcrypt.checkpw(password.encode('utf-8'), hashed_password.encode('utf-8')):
                        c.execute(queries_general.get_user_firstname, {'pseudo': pseudo})
                        ...
                        if firstname_result is not None:
                            ...
                            if table_result:
                                table_name, pseudo = table_result
                                # Générer le token de connexion en utilisant le nom de la table
                                payload = {'pseudo': pseudo, 'user_type': table_name}
