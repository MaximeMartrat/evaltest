    ...
    if method == 'POST':
        try:
            ...
            with mysql.connector.connect(**connection_params) as db:
                with db.cursor() as c:
                    c.execute(queries_general.select_table, {'pseudo': pseudo})
                    result = c.fetchone()

                    if result:
                        table_name, pseudo = result
                        if table_name == 'administrateur':
                            update_password = queries_general.update_password_admin
                        elif table_name == 'formateur':
                            update_password = queries_general.update_password_formateur
                        elif table_name == 'salarie':
                            update_password = queries_general.update_password_salarie
                        elif table_name == 'apprenant':
                            update_password = queries_general.update_password_apprenant
                        else:
